...
pub fn set_decimals(decimals: U256) {
    ...
    set_key(BUSD_EQ_YODAS_PER_WISE, U256::from(10).pow(decimals));
}

pub fn yodas_per_wise() -> U256 {
    get_key(BUSD_EQ_YODAS_PER_WISE).unwrap_or_default()
}

pub fn latest_busd_equivalent() -> U256 {
    get_key(BUSD_EQ_LATEST_BUSD_EQUIVALENT).unwrap_or_default()
}
pub fn set_latest_busd_equivalent(latest_busd_equivalent: U256) {
    set_key(BUSD_EQ_LATEST_BUSD_EQUIVALENT, latest_busd_equivalent);
}
